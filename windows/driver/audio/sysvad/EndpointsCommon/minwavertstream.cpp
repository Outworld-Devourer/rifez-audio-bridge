...
--*/
{
    ...
    {
        static LONG g_RifeZBufferInitialized = 0;

        if (InterlockedCompareExchange(&g_RifeZBufferInitialized, 1, 0) == 0)
        {
            NTSTATUS ringStatus = RifeZPcmServiceInitialize();
            if (!NT_SUCCESS(ringStatus))
            {
                DPF(D_TERSE, ("RifeZ: failed to initialize PCM service, 0x%x", ringStatus));
                return ringStatus;
            }
        }
...
)
{
    ...
    else
    {
      ...
      //  if (!g_DoNotCreateDataFiles)
        //{
            // Always process render bytes for bridge capture.
            // WAV generation is optional and handled inside ReadBytes().
            // IMPORTANT:
            // ReadBytes() is not only for WAV dumping. It also forwards render PCM into the
            // RifeZ bridge/ring-buffer path. Do not gate the ReadBytes() call on
...
{
    ULONG bufferOffset = m_ullLinearPosition % m_ulDmaBufferSize;
    static LONG g_RifeZDebugCounter = 0;
    RIFEZ_PCM_INFO info = {};

    while (ByteDisplacement > 0)
    {
        ULONG runWrite = min(ByteDisplacement, m_ulDmaBufferSize - bufferOffset);
        PBYTE pChunk = m_pDmaBuffer + bufferOffset;

        if (!g_DoNotCreateDataFiles)
        {
            m_SaveData.WriteData(pChunk, runWrite);
        }

        RifeZPcmServiceWrite(pChunk, runWrite);

        
        LONG counter = InterlockedIncrement(&g_RifeZDebugCounter);
        if ((counter % 5) == 0)
        {

            RifeZPcmServiceGetInfo(&info);
            DPF(D_TERSE, ("[RifeZ] wrote %lu bytes, buffered=%lu", runWrite, info.BufferedBytes));
        }
