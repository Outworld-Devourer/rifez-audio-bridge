...
--*/
    ...
    ntStatus = RifeZControlDeviceCreate(DriverObject);
    IF_FAILED_ACTION_JUMP(
        ntStatus,
        DPF(D_ERROR, ("RifeZControlDeviceCreate failed, 0x%x", ntStatus)),
        Done);

    //
    // Preserve original dispatch handlers for PortCls-managed devices.
    // Our wrappers will only handle the dedicated RifeZ control device and
    // pass all other device objects through to these originals.
    //
    g_RifeZOriginalCreateDispatch = DriverObject->MajorFunction[IRP_MJ_CREATE];
    g_RifeZOriginalCloseDispatch = DriverObject->MajorFunction[IRP_MJ_CLOSE];
    g_RifeZOriginalDeviceControlDispatch = DriverObject->MajorFunction[IRP_MJ_DEVICE_CONTROL];

    DriverObject->MajorFunction[IRP_MJ_CREATE] = RifeZDispatchCreate;
    DriverObject->MajorFunction[IRP_MJ_CLOSE] = RifeZDispatchClose;
    DriverObject->MajorFunction[IRP_MJ_DEVICE_CONTROL] = RifeZDispatchDeviceControl;
